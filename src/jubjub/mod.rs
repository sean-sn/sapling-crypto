    ...
    pub fn new_with_window_size(window_size: u32) -> Self {
        ...
        // Create the exp table precomputations for the Pedersen hash generators
        {
            ...
            for g in &tmp_params.pedersen_hash_exp {
                ...
                let mut base_bytes = unsafe {
                    slice::from_raw_parts_mut((&mut base as *mut _) as *mut u64,
                                              16)
                };
                for t in g {
                    let mut table = Vec::with_capacity(1 << window);
                    for p in t {
                        let p2 = unsafe {
                            slice::from_raw_parts((p as *const _) as *const u64,
                                                  16)
                        };
                        twisted_edwards_jubjub_add::ext_twisted_ed_precomp_256(
