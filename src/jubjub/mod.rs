...
impl JubjubBls12 {
    pub fn new() -> Self {
        Self::new_with_n_segments_and_window_size(5, 8, None)
    }

    pub fn new_with_window_size(window_size: u32) -> Self {
        Self::new_with_n_segments_and_window_size(5, window_size, None)
    }
        ...
        .map(|mut generator| {
            ...
            while n_bits <= fs::Fs::NUM_BITS {
                ...
                for _ in 0..(1 << window_size) {
                    ...
                    base = base.add(&generator, &JUBJUB_BLS12_PARAMS_WITH_EDWARDS_D);
                }
                ...
                for _ in 0..window_size {
                    generator = generator.double(&JUBJUB_BLS12_PARAMS_WITH_EDWARDS_D);
                }
            }
...
/// not exist.
fn create_exp_table_path_path_dirs_if_dne(path: &str) -> io::Result<()> {
...
fn write_exp_table_to_file(
    ...
    create_exp_table_path_path_dirs_if_dne(exp_table_path)
        .expect("failed to create dirs in exp-table path");

    let mut file = OpenOptions::new()
        .create(true)
        .write(true)
        .open(exp_table_path)
        .expect("failed to open/create Pedersen Hash params file");
    ...
    file.write_all(&n_segments_as_bytes).expect("failed to write n_segments");
    file.write_all(&window_size_as_bytes).expect("failed to write window_size");
    ...
    while n_segments_written < n_segments {
        ...
        for segment_table in exp_table {
            ...
            file.write_all(&segment_table_bytes).expect("failed to write segment table bytes");
        }
...
pub fn read_exp_table_range(
    ...
    let file = File::open(exp_table_path).expect("failed to open Pedersen Hash params file");
    ...
    let window_size = {
        let mut buf = [0u8; 4];
        reader.read_exact(&mut buf).expect("failed to read bytes 4-8 from the exp-table file");
        u32::from_le_bytes(buf)
    };
    ...
    reader.seek(SeekFrom::Current(n_exp_table_bytes_to_skip)).unwrap();
